...
- Toss 승인 API는 외부 시스템 호출로 인한 실패 확률이 높음
- 승인 이후 포인트 충전 및 상태 반영이 DB 수준에서 확실히 처리되기 위해선 별도 트랜잭션이 필요함
- 두 단계 분리를 통해 승인 완료 → 충전 실패 → 복구 가능 구조를 구현

<br>

> ✅ 결과적으로 시스템 전반의 복구 가능성, 신뢰성 상승
