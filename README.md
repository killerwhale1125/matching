...
⚠️ 참고: DB 동기화 스케줄링 및 예외처리는 구현하였지만 Redis 장애 시 조회수 손실 복구 스케줄러는 시간상의 이유로 구현하지 못했습니다.
...
이에 따라,
- 기본적으로는 최신 등록순으로 페이지네이션된 프로필을 제공하고,
- 사용자가 원하는 경우 해당 페이지 내에서만 조회수순, 이름 가나다순 등으로 정렬할 수 있도록 설계하였습니다.

**🔹 이 방식의 장점**
- *조회수 캐싱 구조와 충돌 없음* : 조회수는 Redis에 캐싱되어 DB와 완전히 동기화되지 않아도 무방
- 단일 Index 효율 증가 : 기본 정렬은 DB Index의 생성일 기준으로 설계하였으며, 추가적인 Index 테이블 컬럼 관리 불필요로 INSERT/UPDATE/DELETE 성능 향상
- 정렬의 유연성 확보 : 사용자가 실제 보는 페이지 단위에서만 정렬이 이루어지므로 UX를 해치지 않으면서도 정렬 기준을 다양하게 지원
